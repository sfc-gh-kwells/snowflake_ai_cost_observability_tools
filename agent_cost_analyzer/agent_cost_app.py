...
try:
    ...
    import os
    session = Session.builder.config('connection_name', os.getenv("SNOWFLAKE_CONNECTION_NAME") or "MY_DEMO").create()
...
def calculate_credits(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty:
        return df
    ...
    for model, rates in CREDIT_RATES.items():
        ...
        df.loc[mask, 'INPUT_CREDITS'] = (df.loc[mask, 'INPUT_TOKENS'].fillna(0) / 1_000_000) * rates['input']
        df.loc[mask, 'OUTPUT_CREDITS'] = (df.loc[mask, 'OUTPUT_TOKENS'].fillna(0) / 1_000_000) * rates['output']
        df.loc[mask, 'CACHE_WRITE_CREDITS'] = (df.loc[mask, 'CACHE_WRITE_TOKENS'].fillna(0) / 1_000_000) * rates['cache_write']
        df.loc[mask, 'CACHE_READ_CREDITS'] = (df.loc[mask, 'CACHE_READ_TOKENS'].fillna(0) / 1_000_000) * rates['cache_read']
    
    df['TOKEN_CREDITS'] = df['INPUT_CREDITS'] + df['OUTPUT_CREDITS'] + df['CACHE_WRITE_CREDITS'] + df['CACHE_READ_CREDITS']
    return df

def main():
    st.title("Agent API Cost Analyzer")
    st.caption("Analyze Cortex Agent API costs by user and agent")
    ...
    for idx, row in agents_df.iterrows():
        if row['AGENT_NAME'] in selected_agents:
            events = get_agent_events(row['DB_NAME'], row['AGENT_NAME'], time_range)
            if not events.empty:
                all_events.append(events)
            query_costs = get_analyst_query_costs(row['DB_NAME'], row['AGENT_NAME'], time_range)
    ...
    if not all_events:
        st.info(f"No usage data found for selected agents in the past {time_range} days.")
        return
    
    combined_df = pd.concat(all_events, ignore_index=True)
    combined_df = calculate_credits(combined_df)
    
    query_costs_df = pd.concat(all_query_costs, ignore_index=True) if all_query_costs else pd.DataFrame()
    ...
    total_query_credits = query_costs_df['QUERY_CREDITS'].sum() if not query_costs_df.empty else 0
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["Summary", "By User", "By Agent", "Daily Trend", "Detailed Data"])
    ...
    with tab1:
        ...
        agent_token_credits = filtered_df.groupby('AGENT_NAME')['TOKEN_CREDITS'].sum().reset_index()
        agent_token_credits.columns = ['AGENT_NAME', 'TOKEN_CREDITS']
        
        if not query_costs_df.empty:
            agent_query_credits = query_costs_df.groupby('AGENT_NAME')['QUERY_CREDITS'].sum().reset_index()
            agent_query_credits.columns = ['AGENT_NAME', 'QUERY_CREDITS']
            agent_combined = agent_token_credits.merge(agent_query_credits, on='AGENT_NAME', how='left')
            agent_combined['QUERY_CREDITS'] = agent_combined['QUERY_CREDITS'].fillna(0)
        else:
            agent_combined = agent_token_credits.copy()
            agent_combined['QUERY_CREDITS'] = 0.0
        
        agent_combined['TOTAL_CREDITS'] = agent_combined['TOKEN_CREDITS'] + agent_combined['QUERY_CREDITS']
        agent_combined = agent_combined.sort_values('TOTAL_CREDITS', ascending=False)
        
        st.bar_chart(agent_combined.set_index('AGENT_NAME')[['TOKEN_CREDITS', 'QUERY_CREDITS']])
        st.dataframe(
            agent_combined,
            column_config={
                "AGENT_NAME": "Agent",
        ...
        if not query_costs_df.empty:
            ...
            query_summary.columns = ['AGENT_NAME', 'QUERY_COUNT', 'TOTAL_QUERY_CREDITS', 'TOTAL_ELAPSED_SECONDS']
            st.dataframe(
                query_summary,
                column_config={
                    "AGENT_NAME": "Agent",
    ...
    with tab2:
        ...
        user_summary = user_summary.sort_values('TOKEN_CREDITS', ascending=False)
    ...
    with tab4:
        ...
        st.line_chart(daily_summary.set_index('EVENT_DATE')[['TOKEN_CREDITS', 'INPUT_CREDITS', 'OUTPUT_CREDITS']])
        st.dataframe(
            daily_summary,
            column_config={
                "EVENT_DATE": "Date",
    ...
    with tab5:
        ...
        user_agent_summary = user_agent_summary.sort_values(['EVENT_DATE', 'TOKEN_CREDITS'], ascending=[False, False])
